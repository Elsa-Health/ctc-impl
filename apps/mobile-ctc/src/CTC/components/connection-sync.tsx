...
import {getStorage, onSnapshotUpdate} from '../emr-helpers/store';
...
const ConnectionSync = React.memo(function ({
  provider,
  wsURL,
  crdtCollection,
}: any) {
  ...
  const {socket, status, retry} = useWebSocket({
    ...
    onOpen(socket) {
      ...
      if (socket.readyState === WebSocket.OPEN) {
        fetchCRDTMessages(provider, crdtCollection).then(message => {
          if (message !== null) {
            // console.log(s);
  ...
  React.useEffect(() => {
    if (socket !== undefined && status === 'online') {
      ...
      const sub = onSnapshotUpdate(getStorage(), provider, msg => {
